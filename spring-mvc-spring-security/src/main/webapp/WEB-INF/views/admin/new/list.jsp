...
<body>
	<div class="main-content">
									...
									<div class="table-btn-controls">
										<div class="pull-right tableTools-container">
											...
											<div class="dt-buttons btn-overlap btn-group">
												...
												<span >
													<input type="text" id="title" value ="${saveSearch }" name="title" placeholder="tìm kiếm bài viết" style="height: 30px; margin-right:10px;margin-top:5px" />
		...
		<input type="hidden" value="<%=request.getParameter("title") %>" id="searchString" name="searchString"/>
		</div>
	<script>

		
		var currentPage = ${model.page}; // client truyền về 1 cái page sau đó ta sẽ truyền lại client để xử lý tiếp
		...
		function warningBeforeDelete() {
			swal({
			  ...
			  if (isConfirm.value) {
					var ids = $('#newsList').find('tbody input[type=checkbox]:checked').map(function () {
			            return $(this).val();
			        }).get();
					deleteNew(ids);
			  }
	...
	function deleteNew(data) {
        $.ajax({
            ...
            success: function (result) {
                window.location.href = "${newURL}?page=1&limit=4&message=delete success";
	...
	function toggleDeleteButton() {
	    if ($('#newsList').find('tbody input[type=checkbox]:checked').length > 0) {
